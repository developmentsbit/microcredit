...
<head>
    ...
    <title>মাসিক মুনাফা প্রদান এবং কালেকশান </title>
    ...
    <table  cellpadding="0" cellspacing="0" style="border-bottom: 2px #000 dotted; width: 100%;" >
        <tr>    
            ...
            <td align="center">
                ...
                <label> 
                    <span style="font-size:24px; "> শ্যামল ছায়া সমাজকল্যাণ সংস্থা</span><br>
               
                       

                        <b style="font-size:18px; margin-bottom: 5px; ">মাসিক মুনাফা প্রদান এবং কালেকশান</b><br>
...
<table cellpadding="0" cellspacing="0"   style="width:100%;margin-right:30px; background: #f4f4f4">
            ...
            <tr>
                ...
                <td   style="border-right: 1px #000 solid;border-bottom: 1px #000 solid;" valign="top" align="center">মাসিক মুনাফা<br>প্রদান</td>
                <td style="border-right: 1px #000 solid;border-bottom: 1px #000 solid;" valign="top" align="center">মাসে সঞ্চয় পরিমাণ</td>
                <td style="border-right: 1px #000 solid;border-bottom: 1px #000 solid;" valign="top" align="center"> সঞ্চয় উত্তোলন </td>
            ...
            $selectMember=$db->link->query("SELECT `fixed_deposit_collections`.`collection_date`,sum(`fixed_deposit_collections`.`deposit_ammount`) as 'deposit_ammount',sum(`fixed_deposit_collections`.`return_deposit`) as 'return_deposit',`fixed_deposit_registrations`.`registration_id`,`members`.`aplicant_name`,`members`.`father_name`,`members`.`phone`,`members`.`present_address`,`members`.`member_id` FROM `fixed_deposit_collections` INNER JOIN `fixed_deposit_registrations` ON `fixed_deposit_collections`.`member_id` =`fixed_deposit_registrations`.`registration_id` INNER JOIN `members` ON `fixed_deposit_registrations`.`member_id`=`members`.`member_id` WHERE `fixed_deposit_collections`.`branch_id`='$branch_id' AND `fixed_deposit_registrations`.`schema_id`='$schema_id' AND `fixed_deposit_collections`.`area_id`='$area_id'  AND `fixed_deposit_registrations`.`status`='1' GROUP BY `fixed_deposit_collections`.`member_id` ORDER BY `fixed_deposit_registrations`.`registration_id` ASC limit $lower,$upper");
              ...
              if($selectMember){
                 while($fetch_info=$selectMember->fetch_array())
                {
                    ...
                    $tamount=intval($fetch_info['deposit_ammount']-$fetch_info['return_deposit']);
    ...
    <td style="border-left: 1px #000 solid;border-bottom: 1px #000 solid;">&nbsp;<a href="fixed_deposit_statement.php?regID=<?php print $fetch_info['registration_id'] ?>" target="_blank" tabindex="1000" style="text-decoration: none;" ><?php print $fetch_info['registration_id'] ?></a></td>   
  ...
  <td style="border-left: 1px #000 solid;border-bottom: 1px #000 solid; text-align: right;border-right: 1px #000 solid;"> 
        ...
        if($selectprofit->num_rows>0)
        {
            ...
            $netprofit=round($fetch_Profit[0]-$fetch_Profit[1],2);
            print $netprofit;
        }
...
?>&nbsp;</td>
                   ...
                   <td valign="center" style="border-right: 1px #000 solid;border-bottom: 1px #000 solid; text-align: center;padding: 2px;">
                   	<input type="text" name="return_profit[]" name="return_profit[]"  style="width:100px; height:25px; " autocomplete="off">
                    </td>

                    <td valign="center"style="border-right: 1px #000 solid;border-bottom: 1px #000 solid;text-align: right;padding: 2px;">
                    <input type="text" name="deposit_ammount[]" id="deposit_ammount[]"  style="width:100px; height:25px; " autocomplete="off">
                    </td>

                    <td valign="center" style="border-right: 1px #000 solid;border-bottom: 1px #000 solid;text-align: right; padding: 2px;">

                    <input type="text" name="return_deposit[]" id="return_deposit" style="width:100px; height:25px; " autocomplete="off">
                    <input type="hidden" name="reg_id[]" id="reg_id[]" value="<?php echo $fetch_info['registration_id']; ?>">
                    <input type="hidden" name="branch_id" value="<?php print $branch_id;?>">
                    <input type="hidden" name="areaid" value="<?php print $area_id;?>">
...
<br>
    ...
    <table style="width:100%;">
        ...
        <tr >
            <td colspan="3" align="center"><input type="button" value="Save" name="Save" onclick="return profit_provide_collection()" id="save_data" style="height:35px; width: 160px; background: green; color: #fff; border-radius:5px;" ></td>
        </tr>
    </table>
