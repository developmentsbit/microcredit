            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h5>সঞ্চয় রেজিষ্ট্রেশন রিপোর্ট</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{ url('show_saving_register_report') }}" method="get">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-sm-4 mb-3">
                                    ...
                                    <div class="input-group">
                                        <select class="js-example-basic-single form-control @error('schema_id') is-invalid @enderror" name="schema_id" id="schema_id"  required="">
                                            ...
                                            @php
                                            $schema = DB::table("saving_schemas")->get();
                                            @endphp
                                            <option value="">নির্বাচন করুন</option>
                                            @if(isset($schema))
                                            @foreach($schema as $s)
                                            <option value="{{ $s->id }}">{{ $s->deposit_name }}</option>
                                            @endforeach
                                            @endif
                                        </select>
                                    </div>
